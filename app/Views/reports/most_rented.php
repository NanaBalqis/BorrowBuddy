...
>
  ...
  <body>
                ...
                <div class="card mb-4">
                    ...
                    <table class="table table-bordered text-center mt-4">
                        ...
                        <?php $i = 1; foreach ($data as $row): ?>
                            ...
                            <td><?= $i++ ?></td>
                            <td><?= esc($row->title) ?></td>
                            <td><?= esc($row->times_rented) ?></td>
                            </tr>
                        <?php endforeach ?>
                        </tbody>
                    </table>
                    <canvas id="mostRentedChart" height="100"></canvas>
                    </div>
                </div>
                </div>
        <!-- /Content -->                                  
    ...
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <script>
    function renderMostRentedChart(isPrint = false) {
        const ctx = document.getElementById('mostRentedChart').getContext('2d');
        new Chart(ctx, {
            type: 'bar',
            data: {
                labels: <?= json_encode(array_column($data, 'title')) ?>,
                datasets: [{
                    label: 'Times Rented',
                    data: <?= json_encode(array_column($data, 'times_rented')) ?>,
            ...
            options: {
                plugins: {
                    legend: {
                        labels: { color: isPrint ? 'black' : 'white' }
                    },
                    title: {
                        display: true,
                        text: 'Top Rented Books',
                ...
                scales: {
                    x: {
                        ...
                        grid: { color: isPrint ? '#cccccc' : 'rgba(255,255,255,0.23)', lineWidth: 0.5 }
                    },
                    y: {
                        ticks: { color: isPrint ? 'black' : 'white' },
                        grid: { color: isPrint ? '#cccccc' : 'rgba(255,255,255,0.23)', lineWidth: 0.5 }
