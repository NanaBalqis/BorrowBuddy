...
>
  <head>
    ...
    @media print {
      ...
      body::after {
        content: "Printed on: <?= date('Y-m-d H:i:s') ?>";
        position: fixed;
        bottom: 10px;
        right: 20px;
        font-size: 12px;
      ...
      .btn, .navbar, .sidebar, .print-hidden {
  ...
  <body>
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between">
                        <h4>Monthly Rental Summary</h4>
                        <div>
                            <a href="<?= site_url('report') ?>" class="btn btn-secondary btn-sm">Back</a>
                            <button onclick="printPage()" class="btn btn-primary btn-sm">Print</button>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <tbody>
                                <?php $i = 1; foreach ($data as $row): ?>
                                    <tr>
                                        <td><?= $i++ ?></td>
                                        <td><?= esc($row->month) ?></td>
                                        <td><?= esc($row->total_rentals) ?></td>
                                        <td><?= number_format($row->total_late_fees, 2) ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
                        <canvas id="monthlyRentalChart" height="100"></canvas>
                    </div>
                </div>
            </div>
        <!-- /Content -->                                  
    ...
    <script>
    function renderChartForTheme(isPrint = false) {
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Total Rentals',
                        data: <?= json_encode(array_column($data, 'total_rentals')) ?>,
                        backgroundColor: isPrint ? '#4a90e2' : '#5534dbff',
                        color: 'white'
                    },
                    {
                        label: 'Late Fees (RM)',
                        data: <?= json_encode(array_column($data, 'total_late_fees')) ?>,
                        backgroundColor: isPrint ? '#e74c3c' : '#e74c3c',
                        color: 'white'
                    }
                ]
            },
            options: {
                ...
                scales: {
                    x: {
                        ticks: { color: isPrint ? 'black' : 'white' },
                        grid: {
                            color: isPrint ? '#cccccc30' : 'rgba(255,255,255,0.23)',
                            lineWidth: isPrint ? 1 : 0.5
                        }
                    },
                    y: {
                        ticks: { color: isPrint ? 'black' : 'white' },
    ...
    window.onload = () => {
        renderChartForTheme(false); // screen view
    };

    function printPage() {
        document.getElementById('monthlyRentalChart').remove();
        const canvas = document.createElement('canvas');
        canvas.id = 'monthlyRentalChart';
        canvas.height = 100;
        document.querySelector('.card-body').prepend(canvas);

        renderChartForTheme(true); // print version
        setTimeout(() => window.print(), 100); // wait to render
    }
    </script>
    <?php endif; ?> 
  </body>
