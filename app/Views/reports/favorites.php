...
>
  <head>
    ...
    @media print {
        ...
        .btn, .layout-menu, .layout-navbar, .print-hidden {
  ...
  <body>
            ...
            <div class="card mb-4">
                ...
                <table class="table table-bordered text-center mt-4">
                    ...
                    <?php $i = 1; foreach ($data as $row): ?>
                        ...
                        <td><?= $i++ ?></td>
                        <td><?= esc($row->title) ?></td>
                        <td><?= esc($row->total_favorites) ?></td>
                        </tr>
                    <?php endforeach ?>
                    </tbody>
                </table>
                <canvas id="favoriteBooksChart" style="max-width: 400px; margin: 0 auto;" height="300"></canvas>
                </div>
            </div>
            </div>
        <!-- /Content -->                                  
    ...
    <script>
    function renderFavoriteChart(isPrint = false) {
        const ctx = document.getElementById('favoriteBooksChart').getContext('2d');
        new Chart(ctx, {
            type: 'pie',
            data: {
                labels: <?= json_encode(array_column($data, 'title')) ?>,
                datasets: [{
                    data: <?= json_encode(array_column($data, 'total_favorites')) ?>,
                    backgroundColor: [
                        '#5534dbff',
                        '#4a90e2',
                        '#e74c3c',
    ...
    window.onload = () => {
        renderFavoriteChart(false);
    };

    document.getElementById('printBtn').addEventListener('click', function () {
        document.getElementById('favoriteBooksChart').remove();
        const canvas = document.createElement('canvas');
        canvas.id = 'favoriteBooksChart';
        canvas.height = 100;
        document.querySelector('.card-body').prepend(canvas);

        renderFavoriteChart(true);
