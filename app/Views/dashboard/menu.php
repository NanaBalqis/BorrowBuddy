        ...
        <div class="layout-page">
          ...
          >
            ...
            <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
              <!-- Search -->
              <form method="GET" action="<?= base_url('menu') ?>" class="navbar-nav align-items-center">
            ...
            <div class="row">
              ...
              <div class="col-12 order-0 mb-4">
                  <div class="card-header d-flex align-items-center justify-content-between pb-0">
                      <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Available Books /</span> Menu</h4>
                      <br>
                  </div>
                  <div class="card-body">
                      <div class="container">
                          <form id="filterForm" method="GET" action="<?= base_url('menu') ?>" class="mb-4">
                              <div class="row">
                                  <div class="col-md-4">
                                      <label for="categoryFilter" class="form-label">Genre</label>
                                      <select id="categoryFilter" name="genre" class="form-control">
                                          <option value="">All Genres</option>
                                          <?php if (!empty($genres)) : ?>
                                              <?php foreach ($genres as $genre) : ?>
                                                  <option value="<?= $genre['genre'] ?>" <?= ($selectedGenre == $genre['genre']) ? 'selected' : '' ?>>
                                                      <?= ucfirst($genre['genre']) ?>
                                                  </option>
                                              <?php endforeach; ?>
                                          <?php endif; ?>
                                  ...
                                  <div class="col-md-4">
                                      <label for="priceFilter" class="form-label">Sort by Year</label>
                                      <select id="priceFilter" name="publishedYear" class="form-control">
                                          <option value="">Select</option>
                                          <option value="asc" <?= ($selectedPublishedYearOrder == 'asc') ? 'selected' : '' ?>>Old to New</option>
                                          <option value="desc" <?= ($selectedPublishedYearOrder == 'desc') ? 'selected' : '' ?>>New to Old</option>
                                      </select>
                                  </div>
                                  <div class="col-md-4">
                                      <label for="modeFilter" class="form-label">Author</label>
                                      <select id="modeFilter" name="author" class="form-control">
                                          <option value="">All Authors</option>
                                          <?php if (!empty($books)) : ?>
                                              <?php foreach ($books as $book) : ?>
                                                  <option value="<?= $book['author'] ?>" <?= ($selectedAuthor == $book['author']) ? 'selected' : '' ?>>
                                                      <?= ucfirst($book['author']) ?>
                                                  </option>
                                              <?php endforeach; ?>
                                          <?php endif; ?>
                              ...
                              <?php foreach ($books as $book) : ?>
                                  <div class="col">
                                      <div class="card h-100 position-relative">
                                          <!-- Favorite Icon -->
                                          <form method="POST" class="favorite-form position-absolute top-0 end-0 m-2" style="z-index: 10;">
                                              <input type="hidden" name="bookId" value="<?= $book['bookId'] ?>">
                                              <input type="hidden" name="title" value="<?= esc($book['title']) ?>">
                                              <?= csrf_field() ?>
                                              <button type="submit" class="btn p-0 border-0 bg-transparent favorite-btn" data-book-id="<?= $book['bookId'] ?>" title="Add to Favorites">
                                                  <i class="bi <?= in_array($book['bookId'], $favoriteBookIds ?? []) ? 'bi-heart-fill text-danger' : 'bi-heart text-muted' ?>" style="font-size: 1.5rem;"></i>
                                              </button>
                                          </form>

                                          <div class="card-body">
                                              <div class="d-flex justify-content-center mb-3">
                                                  <img src="<?= $book['book_cover'] ?>" alt="<?= $book['title'] ?>" class="rounded" height="200" width="auto">
                                              </div>
                                              <div class="d-flex align-items-center mb-3">
                                                  <div>
                                                      <h5 class="mb-0"><?= esc(substr($book['title'], 0, 35)) . (strlen($book['title']) > 35 ? '...' : '') ?></h5>
                                                      <small class="text-muted"><?= ucfirst($book['genre']) ?></small>
                                                  </div>
                                              </div>
                                              <div class="user-progress d-flex align-items-center gap-1 mb-2">
                                                  <h6 class="mb-0"><strong>Available Copies:</strong> <?= $book['availableCopies'] ?></h6>
                                              </div>
    ...
    <!--buttonfav-->
      ...
      document.querySelectorAll('.favorite-form').forEach(form => {
          form.addEventListener('submit', function (e) {
              e.preventDefault();
              const bookId = this.querySelector('input[name="bookId"]').value;
              const title = this.querySelector('input[name="title"]').value;
              const icon = this.querySelector('i');

              fetch("<?= base_url('add-favorite') ?>", {
                  method: "POST",
                  headers: {
