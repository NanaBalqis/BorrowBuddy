        ...
        <div class="layout-page">
          ...
          >
            ...
            <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
              <!-- search -->
              <form action="<?= site_url('book') ?>" method="get" class="navbar-nav align-items-center">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-3">
                                <select name="genre" class="form-select">
                                    ...
                                    <?php foreach ($genres as $g): ?>
                                        <option value="<?= esc($g['genre']) ?>" <?= ($selectedGenre == $g['genre']) ? 'selected' : '' ?>><?= esc($g['genre']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <select name="author" class="form-select">
                                    ...
                                    <?php foreach ($authors as $a): ?>
                                        <option value="<?= esc($a['author']) ?>" <?= ($selectedAuthor == $a['author']) ? 'selected' : '' ?>><?= esc($a['author']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <input type="text" name="search" class="form-control" placeholder="Search by title or author..." value="<?= esc($searchQuery) ?>">
                            </div>
                            <div class="col-md-2">
                                <select name="publishedYear" class="form-select">
                                    <option value="">Sort by Year</option>
                                    <option value="asc" <?= ($selectedPublishedYearOrder == 'asc') ? 'selected' : '' ?>>Oldest First</option>
                                    <option value="desc" <?= ($selectedPublishedYearOrder == 'desc') ? 'selected' : '' ?>>Newest First</option>
                                </select>
                            </div>
                            <div class="col-md-1">
                        ...
                        <div class="mt-2 mb-3 d-flex justify-content-end">
                            <a href="<?= site_url('user/addbookpage') ?>" class="btn btn-primary">Add Book</a>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped text-center align-middle">
                                    ...
                                    <?php foreach ($books as $book): ?>
                                        <tr>
                                            <td><?= esc($book['bookId']) ?></td>
                                            <td><?= esc($book['title']) ?></td>
                                            <td><?= esc($book['author']) ?></td>
                                            <td><?= esc($book['genre']) ?></td>
                                            <td>
                                                <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#bookDetailModal<?= $book['bookId'] ?>">
                                                    View Detail
                                                </button>
                                                <!-- Modal -->
                                                <div class="modal fade" id="bookDetailModal<?= $book['bookId'] ?>" tabindex="-1" aria-labelledby="bookDetailModalLabel<?= $book['bookId'] ?>" aria-hidden="true">
                                                  <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                      <div class="modal-header">
                                                        <h5 class="modal-title" id="bookDetailModalLabel<?= $book['bookId'] ?>">Book Detail</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                      </div>
                                                      <div class="modal-body text-center">
                                                        <?php if (!empty($book['book_cover'])): ?>
                                                            <img src="<?= $book['book_cover'] ?>" alt="Book Cover" style="width:120px;height:170px;object-fit:cover;" class="mb-3">
                                                        <?php else: ?>
                                                            <div class="mb-3 text-muted">No Image</div>
                                                        <?php endif; ?>
                                                        <div class="mb-2"><strong>Title:</strong> <?= esc($book['title']) ?></div>
                                                        <div class="mb-2"><strong>Author:</strong> <?= esc($book['author']) ?></div>
                                                        <div class="mb-2"><strong>Genre:</strong> <?= esc($book['genre']) ?></div>
                                                        <div class="mb-2"><strong>Year:</strong> <?= esc($book['publishedYear']) ?></div>
                                                        <div class="mb-2"><strong>Description:</strong><br> <?= esc($book['description']) ?></div>
                                                      </div>
                                                    </div>
                                                  </div>
                                                </div>
                                            ...
                                            <td>
                                                <form method="post" action="<?= site_url('user/update-book/' . $book['bookId']) ?>">
                                                    <a href="<?= site_url('user/edit-book/' . $book['bookId']) ?>" class="btn btn-warning btn-sm">Edit</a>
                                                    <a href="<?= site_url('book/delete/' . $book['bookId']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this book?')">Delete</a>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
