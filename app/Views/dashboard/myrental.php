          ...
          <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="row">
                <div class="col-12 order-1 mb-4">
                    ...
                    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">My Rented Books /</span> Details</h4>
                    </div>
                    <div class="card-body">
                    <div class="container">
                        <?php if (empty($rentalClasses)): ?>
                        ...
                        <?php
                            ...
                            foreach ($rentalClasses as $rented) {
                                if ($rented['status'] === 'rented' && $rented['dueDate'] < $currentDate) {
                                    $overdueBooks[] = $rented;
                                } else {
                                    $rentedBooks[] = $rented;
                                }
                        ...
                        <?php if (!empty($rentedBooks)): ?>
                            <div class="card mb-4">
                            <h3 class="card-header bg-pastel-green">Rented Books</h3>
                            </div>
                            <hr>
                            <?php foreach ($rentedBooks as $rented): ?>
                            <div class="mb-4">
                                <h5><?= $rented['title'] ?></h5>
                                <p><strong>Author:</strong> <?= $rented['author'] ?></p>
                                <p><strong>Genre:</strong> <?= $rented['genre'] ?></p>
                                <p><strong>Status:</strong> <?= ucfirst($rented['status']) ?></p>
                                <p><strong>Rent Date:</strong> <?= $rented['rentalDate'] ?></p>
                                <p><strong>Due Date:</strong> <?= $rented['dueDate'] ?></p>
                                <p><strong>Return Date:</strong> <?= $rented['returnDate'] ?></p>
                                <div class="d-flex justify-content-start">
                                <button type="button" class="btn btn-info me-2" data-bs-toggle="modal" data-bs-target="#courseModal<?= $rented['rentalId'] ?>">View More</button>
                                </div>
                                <hr>
                            </div>

                            <!-- Modal -->
                            <div class="modal fade" id="courseModal<?= $rented['rentalId'] ?>" tabindex="-1" aria-labelledby="courseModalLabel<?= $rented['rentalId'] ?>" aria-hidden="true">
                                <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    <h5 class="modal-title" id="courseModalLabel<?= $rented['rentalId'] ?>"><?= $rented['title'] ?></h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                    <div class="d-flex justify-content-center mb-3">
                                        <img src="<?= $rented['book_cover'] ?>" alt="<?= $rented['title'] ?>" class="rounded" height="300" width="auto">
                                    </div>
                                    <p><strong>Genre:</strong> <?= ucfirst($rented['genre']) ?></p>
                                    <p><strong>Total Copies:</strong> <?= $rented['totalCopies'] ?></p>
                                    <p><strong>Description:</strong> <?= $rented['description'] ?? 'N/A' ?></p>
                                    <p><strong>Available:</strong> <?= $rented['availableCopies'] ?></p>
                                    <p><strong>Status:</strong> <?= ucfirst($rented['status']) ?></p>
                                    <p><strong>Rent Date:</strong> <?= $rented['rentalDate'] ?></p>
                                    <p><strong>Due Date:</strong> <?= $rented['dueDate'] ?></p>
                                    <p><strong>Return Date:</strong> <?= $rented['returnDate'] ?></p>
                                    <p><strong>Late Fee:</strong> RM<?= $rented['lateFee'] ?></p>
                                    </div>
                        ...
                        <?php if (!empty($overdueBooks)): ?>
                            <div class="card mb-4">
                            <h3 class="card-header bg-pastel-red">Overdue Rentals</h3>
                            </div>
                            <hr>
                            <?php foreach ($overdueBooks as $rented): ?>
                            <div class="mb-4">
                                <h5><?= $rented['title'] ?></h5>
                                <p><strong>Author:</strong> <?= $rented['author'] ?></p>
                                <p><strong>Genre:</strong> <?= $rented['genre'] ?></p>
